...
if __name__ == "__main__":
    ...
    if 0:
        ...
        camera_start_id = len(glob.glob(rep_dir+'train/*.png'))
        print('==> copy pre to pid dataset from camera id:',camera_start_id)
        process_dataset(pre_dir+'fix_train_list.txt',  pre_dir+'train_set/',  save_dir+'pid_dataset/',camera_start_id=camera_start_id)
    ...
    pre_train = pd.read_csv(pre_dir+'fix_train_list.txt',sep=' ',header=None,names=['filename','identity'])
    rep_train = pd.read_csv(rep_dir+'fix_train_list.txt',sep=' ',header=None,names=['filename','identity'])
    pre_train['filename'] = pre_train['filename'].apply(lambda x: x.split('/')[1]) 
    ...
    pre_pids = pre_train['identity'].astype(str).tolist()
    pre_pids = list(set(pre_pids))
    ...
    if 0:
        ...
        with tqdm(total = len(rep_pids)) as pbar:
            for pid in rep_pids:
                ...
                pbar.update(1)
        # trainVal
        print('==> copy to all trainVal')
        trainVal_path = save_dir + 'all_trainVal/'
        os.makedirs(trainVal_path,exist_ok=True)

        with tqdm(total = len(all_pids)) as pbar:
            for pid in all_pids:
                imgs = os.listdir(os.path.join(root_path, pid))
                for img in imgs:
                    shutil.copy(os.path.join(os.path.join(root_path, pid), img), os.path.join(trainVal_path, img))
    ...
    if 1:
        ...
        train_all_pids = pre_pids + train_rep_pids
        ...
        print('==> copy to all train')
        train_path = save_dir + 'all_f{}_train/'.format(kfold)
        train2_path = save_dir + 'all_f{}_train2/'.format(kfold)
        train4_path = save_dir + 'all_f{}_train4/'.format(kfold)

        os.makedirs(train_path,exist_ok=True)
        os.makedirs(train2_path,exist_ok=True)
        os.makedirs(train4_path,exist_ok=True)
        with tqdm(total = len(train_all_pids)) as pbar:
            for pid in train_all_pids:
                imgs = os.listdir(os.path.join(root_path, pid))
                if 1:
                    for img in imgs:
                        shutil.copy(os.path.join(os.path.join(root_path, pid), img), os.path.join(train_path, img))
