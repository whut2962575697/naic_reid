...
class HistLabelTrainer(object):
    ...
    def step(self, batch):
        ...
        loss,tpl,ce,hlce = self.loss_func(outputs, target,histlabels,in_detail=True)
        ...
        if self.current_iteration % self.cfg.SOLVER.TENSORBOARD.LOG_PERIOD == 0:
            if self.summary_writer:
                ...
                self.summary_writer.add_scalar('Train/hlce',hlce,self.current_iteration)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_dl, total=len(self.val_dl),
                ...
                ff,histpred = self.model(data,output_feature='with_histlabel')
                ff = ff.data.cpu()
        ...
        hist_acc = (histpreds[:histlabels.size()[0]].max(1)[1] == histlabels).float().mean().item()
        ...
        if self.summary_writer:
            self.summary_writer.add_scalar('Valid/hist_acc',hist_acc,self.train_epoch)
